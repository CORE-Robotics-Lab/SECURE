...
class NPO(RLAlgorithm):
    ...
    def __init__(self,
        ...
        self.generator_train_itrs = generator_train_itrs
        ...
        self._old_policy = policy.clone(f'old_policy_{index}')
        ...
        for _ in trainer.step_epochs():
            ...
            if is_save_checkpoint:
                if trainer.step_itr % 10 == 0:
                    checkpoint_path = ""
                    saver_checkpoint = tf.train.Saver(self.save_dict)
                    saver_checkpoint.save(tf.compat.v1.get_default_session(), checkpoint_path)
    ...
    def compute_irl(self, paths, itr=0):
        tot_rew = 0
        tot_positive_confidence_rew = 0
        positive_confidence_num = 0

        for path in paths:
            tot_positive_confidence_rew += np.sum(path['rewards'])
            positive_confidence_num += 1
            tot_rew += np.sum(path['rewards'])
            path['rewards'] *= 0

        tabular.record('Total_rwd', tot_positive_confidence_rew / float(positive_confidence_num))
    ...
    def _train_once(self, itr, episodes):
        ...
        if self.center_grads:
            if self.center_reward_gradients is None:
                self.center_reward_gradients = self.irl_model.center_reward_gradients
            else:
                assert self.center_reward_gradients.keys() == self.irl_model.center_reward_gradients.keys()
                for key in self.center_reward_gradients.keys():
                    self.center_reward_gradients[key] += \
                    self.irl_model.center_reward_gradients[key]
        obs = [
