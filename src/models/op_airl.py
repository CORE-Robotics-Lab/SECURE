...
class OP_AIRL(SingleTimestepIRL):
    """
        ...
        with tf.variable_scope(name) as _vs:
            ...
            with tf.variable_scope('discrim') as dvs:
                ...
                with tf.variable_scope('reward'):
                    ...
                    # for use of adding noise adjustment to reward given to policy
                    # self.reward_noise_added = tf.log(tf.expand_dims(1.0 - self.noise_t, axis=1)) + self.reward
                ...
                self.log_p_tau_original = log_p_tau
                # log_p_tau = tf.expand_dims(1.0 - self.noise_t, axis=1) * log_p_tau - np.log(2) * self.dU  # inside c
                # log_p_tau = tf.log(tf.expand_dims(1.0 - self.noise_t, axis=1)) + log_p_tau  # outside c
                self.log_p_tau_after_noise_adjust = log_p_tau
            ...
            self.cent_loss = -tf.reduce_mean(
                self.labels * (log_p_tau - log_pq) + (1 - self.labels) * (log_q_tau - log_pq) * (tf.exp(log_q_tau - tf.log(
                        expanded_noise_t * 0.5 ** self.dU + (1 - expanded_noise_t) * tf.exp(log_q_tau) + 1e-6))))
            # with tf.control_dependencies([tf.print(tf.reduce_any(tf.is_nan(tf.exp(log_q_tau) / (
            #                     expanded_noise_t * 0.5 ** self.dU + (1 - expanded_noise_t) * tf.exp(log_q_tau)))))]):
            #     self.cent_loss = -tf.reduce_mean(
            #         self.labels * (log_p_tau - log_pq) + (1 - self.labels) * (log_q_tau - log_pq) * (tf.exp(log_q_tau) / (
            #                     expanded_noise_t * 0.5 ** self.dU + (1 - expanded_noise_t) * tf.exp(log_q_tau))))
            # self.cent_loss = -tf.reduce_mean(self.labels * (log_p_tau - log_pq) + (1 - self.labels) * (log_q_tau - log_pq))
    ...
    def fit(self, paths, policy=None, batch_size=32, logger=None, lr=1e-3, **kwargs):
        ...
        # self._insert_zero_noise(paths)
        # self._insert_constant_noise(paths, -0.1)
        ...
        if logger:
            ...
            discrim_acc, \
            log_p_tau_original, \
            log_p_tau_after_noise_adjust = \
                tf.get_default_session().run([self.reward,
                                              self.value_fn,
                                              self.discrim_output,
                                              self.discriminator_acc,
                                              self.log_p_tau_original,
                                              self.log_p_tau_after_noise_adjust],
            ...
            with logger.tabular_prefix("AIRL_owntau | "):
                ...
                logger.record_tabular('log_P', np.mean(log_p_tau_original))
                logger.record_tabular('log_P_after_noise_adjust', np.mean(log_p_tau_after_noise_adjust))
    ...
    def eval(self, paths, **kwargs):
        ...
        if self.score_discrim:
            self._compute_path_probs(paths, insert=True)
            # self._insert_constant_noise(paths, 0.0)
