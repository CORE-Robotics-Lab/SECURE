...
def network_cbf_batch(x, r):
    ...
    Args:
        x: [bs, topk + 1, topk + 1, 2] matrix (bs is batch size)
        r: threshold

    Returns:
        h: CBF value h(agent, obstacle). Shape: [bs, topk + 1, 1].
    ...
    x = tf.concat([x,
        ...
        tf.expand_dims(d_norm - r, 2)], axis=3)
    ...
    x = x[:, -1:, :, :]  # shape: [bs, 1, 13, 4]. The last element in dim1 is 0 because it's agent-agent
    mask = mask[:, -1:, :, :]  # shape: [bs, 1, 13, 1]
    ...
    x = tf.contrib.layers.conv1d(inputs=x,
                                 ...
                                 activation_fn=None)
    # x shape: [bs, 1, 13, 1]
    h = x * mask  # filter out too far away obs, h shape: [bs, 1, 13, 1]
    h = tf.reshape(h, [bs, -1])  # h: [bs, 13]
    return h, mask


# TODO: add bs
def loss_barrier_batch(h, dang_mask_reshape, safe_mask_reshape, eps=[1e-3, 0]):
    """
    Description:
    ...
    Args:
        ...
        dang_mask_reshape (bs, topk + 1): mask for dangerous states.
        safe_mask_reshape (bs, topk + 1): mask for safe states.
        eps: threshold
    ...
    # h shape to [bs, topk + 1,]
    h_reshape = h

    # Form dangerous and safe h
    dang_h = tf.boolean_mask(h_reshape, dang_mask_reshape)  # [bs, topk + 1]
    safe_h = tf.boolean_mask(h_reshape, safe_mask_reshape)

    # Get number of dangerous and safe h
    num_dang = tf.cast(tf.shape(dang_h)[1], tf.float32)  # topk + 1
    num_safe = tf.cast(tf.shape(safe_h)[1], tf.float32)

    # Calculate dangerous and safe loss
    loss_dang = tf.reduce_sum(
        tf.math.maximum(dang_h + eps[0], 0), axis=1) / (1e-5 + num_dang)  # [bs]
    loss_safe = tf.reduce_sum(
        tf.math.maximum(-safe_h + eps[1], 0), axis=1) / (1e-5 + num_safe)  # [bs]
    loss_dang, loss_safe = tf.reduce_mean(loss_dang), tf.reduce_mean(loss_safe)

    # Calculate dangerous and safe accuracy
    acc_dang = tf.reduce_sum(tf.cast(
        tf.less_equal(dang_h, 0), tf.float32), axis=1) / (1e-5 + num_dang)  # [bs]
    acc_safe = tf.reduce_sum(tf.cast(
        tf.greater(safe_h, 0), tf.float32), axis=1) / (1e-5 + num_safe)  # [bs]
    ...
    # TODO: think about the init nn training issue
    return loss_dang, loss_safe, acc_dang, acc_safe



# yy: currently don't need above 2 functions
def loss_derivatives_min_batch(s, a, h, s_next, alpha, r, eps=[1e-3, 0]):
    ...
    Args:
        s (bs, topk + 1, 2): current timestep state
        a (bs, 1, 2): current timestep action of the agent
        h (bs, topk + 1): the control barrier function
        s_next (bs, topk + 1, 2): next timestep state
        alpha: alpha CBF
        r: threshold
        eps: threshold
    ...
    dsdt = a
    s_agent_next = s_agent + dsdt * config.TIME_STEP  # [bs, 1, 2]

    # Get next timestep state of obstacles
    s_obstacles_next = s_next[:, :-1, :]  # [bs, topk, 2]
    ...
    h_next = tf.compat.v1.map_fn(fn=lambda t: tf.reshape(network_cbf(x=t, r=r)[0], [-1]), elems=x_next)  # [bs, topk + 1]
    # h_next, _ = network_cbf_batch(x=x_next, r=r)  # h_next: [bs, topk + 1]
...
def network_cbf_batch(x, r, name='cbf', trainable=True):
    """
    Description:
        CBF Neural Netwrok
...
def rnd_model_panda(x, name='cbf', trainable=True):
...
def loss_barrier(h, dang_mask_reshape, safe_mask_reshape, eps=[1e-2, 2e-3]):  # ori: eps=[1e-3, 0]
    """
    Description:
        Build the loss function for the control barrier functions.
...
def loss_barrier_bootstrap_al(h, dang_mask_reshape, safe_mask_reshape, eps=[1e-3, 0]):
    """
    Description:
        Build the loss function for the control barrier functions.
    ...
    acc_safe = tf.cond(
        tf.greater(num_safe, 0), lambda: acc_safe, lambda: -tf.constant(1.0))
    acc = tf.reduce_sum(tf.cast(tf.concat([tf.less_equal(dang_h, 0), tf.greater(safe_h, 0)], axis=0), tf.float32)) \
          / (1e-5 + num_dang + num_safe)


    return loss_dang, loss_safe, acc_dang, acc_safe, acc



def loss_barrier_batch(h, dang_mask_reshape, safe_mask_reshape, bs, eps=[1e-3, 0]):
    """
    Description:
        Build the loss function for the control barrier functions.

    Args:
        h (bs, topk + 1, 1): The control barrier function.
        idx: index of the dangerous state. -1 if no dangerous state.
        eps: threshold
    ...
    # h shape to [bs, topk + 1,]
    h_reshape = tf.reshape(h, [bs, -1])

    # Form dangerous and safe h [bs, topk + 1]
    dang_h = tf.boolean_mask(h_reshape, dang_mask_reshape)
    safe_h = tf.boolean_mask(h_reshape, safe_mask_reshape)
...
def loss_derivatives(s, a, h, s_next, alpha, r, eps=[1e-3, 0]):
    ...
    # Get next timestep state of obstacles
    s_obstacles_next = s_next[:-1, :]  # [topk, 2]
