...
class GymEnv(Environment):
    ...
    def __init__(self, env, is_image=False, max_episode_length=None, is_panda=True):
        ...
        """
        self._env = None
        self.is_panda = is_panda
        if isinstance(env, str):
            self._env = gym.make(env)
        elif isinstance(env, gym.Env) or isinstance(env, gymnasium.Env):
            self._env = env
            # # yy: set seed
        ...
        # self._observation_space = akro.from_gym(self._env.observation_space['observation'], is_image=is_image)
