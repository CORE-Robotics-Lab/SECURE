...
class GaussianMLPPolicy(GaussianMLPModel, Policy):
    ...
    def get_actions(self, observations):
        ...
        samples, means, log_stds = self._f_dist(np.expand_dims(
            observations, 1))
        samples = self.action_space.unflatten_n(np.squeeze(samples, 1))  # [bs, 1, action_space_shape]
        means = self.action_space.unflatten_n(np.squeeze(means, 1))  # [bs, 1, action_space_shape]
        log_stds = self.action_space.unflatten_n(np.squeeze(log_stds, 1))  # [bs, 1, action_space_shape]
