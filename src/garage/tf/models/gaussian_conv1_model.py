...
class GaussianConv1Model(Model):
    """GaussianCNNModel.
    ...
    def _build(self, state_input, name=None):
        ...
        with tf.compat.v1.variable_scope('dist_params'):
            ...
            # mean_network
            x = tf.contrib.layers.conv1d(inputs=state_input,
                                         num_outputs=64,
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='mean_network/conv_1',
                                         activation_fn=tf.nn.relu)
            x = tf.contrib.layers.conv1d(inputs=x,
                                         num_outputs=128,
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='mean_network/conv_2',
                                         activation_fn=tf.nn.relu)
            x = tf.contrib.layers.conv1d(inputs=x,
                                         num_outputs=64,
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='mean_network/conv_3',
                                         activation_fn=tf.nn.relu)
            mean_network = tf.contrib.layers.conv1d(inputs=x,
                                         num_outputs=1,
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='mean_network/conv_4',
                                         activation_fn=None)
            ...
            x = tf.contrib.layers.conv1d(inputs=state_input,
                                         ...
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='log_std_network/conv_1',
                                         activation_fn=tf.nn.relu)
            x = tf.contrib.layers.conv1d(inputs=x,
                                         num_outputs=128,
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='log_std_network/conv_2',
                                         activation_fn=tf.nn.relu)
            x = tf.contrib.layers.conv1d(inputs=x,
                                         num_outputs=64,
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='log_std_network/conv_3',
                                         activation_fn=tf.nn.relu)
            log_std_network = tf.contrib.layers.conv1d(inputs=x,
                                         num_outputs=1,
                                         kernel_size=1,
                                         reuse=tf.AUTO_REUSE,
                                         scope='log_std_network/conv_4',
                                         activation_fn=None)


        mean_var = tf.squeeze(mean_network)
        std_param = tf.squeeze(log_std_network)
