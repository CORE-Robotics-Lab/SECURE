...
class RobotTaskEnvNewObv(gym.Env):
    """Robotic task goal env, as the junction of a task and a robot.
    ...
    def __init__(self, robot: PyBulletRobot, task: Task) -> None:
        ...
        achieved_goal_shape = self.task.get_achieved_goal().astype(np.float32).shape
        desired_goal_shape = self.task.get_goal().astype(np.float32).shape
    ...
    def _get_obs(self) -> Dict[str, np.ndarray]:
        robot_obs = self.robot.get_obs().astype(np.float32)  # robot state
        task_obs = self.task.get_obs().astype(np.float32)  # object position, rotation, velocity, etc...
        observation = np.concatenate([robot_obs, task_obs])
        achieved_goal = self.task.get_achieved_goal().astype(np.float32)
        return observation
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, np.ndarray], float, bool, bool, Dict[str, Any]]:
        ...
        observation = self._get_obs()
        # An episode is terminated iff the agent has reached the target
        terminated = bool(self.task.is_success(self.task.get_achieved_goal().astype(np.float32), self.task.get_goal()))
        info = {"is_success": terminated, "is_fall": self.task.is_fall()[0] > 0.4, "fall_angle": self.task.is_fall()[0]}
        if self.timestep > 150:
            print(">> Timestep reaches 150!")
            terminated = True
