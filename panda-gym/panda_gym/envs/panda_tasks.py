...
from panda_gym.envs.core import RobotTaskEnv, RobotTaskEnvNewObv
...
from panda_gym.envs.tasks.safe_push import SafePush
...
class PandaSafePushEnv(RobotTaskEnvNewObv):
    """Push task wih Panda robot.
    ...
    def __init__(self, render: bool = False, reward_type: str = "sparse", control_type: str = "ee") -> None:
        sim = PyBullet(render=render)
        robot = Panda(sim, block_gripper=True, base_position=np.array([-0.35, 0.0, 0.0]), control_type=control_type)
        task = SafePush(sim, reward_type=reward_type)
        super().__init__(robot, task)
